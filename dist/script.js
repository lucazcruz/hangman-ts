...
class Hangman {
    ...
    $(seletor) { return document.querySelector(seletor); }
    ;
    ...
    wordContainerConstructor(theme, word) {
        let content = '';
        let win = 0;
        [...word].forEach(char => {
      ...
      `;
        ...
        if (win === word.length)
            this.alertMessage("You win! :)");
    }
    async fecthWords() {
        const response = await fetch('../words.json');
        return await response.json();
    }
    async handleWord(theme) {
        const wordsArray = await this.fecthWords();
